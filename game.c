...
void prepareboard(char *daboardfilename)
{
	...
	for(i=0;i<MAXPLAYERS;i++)
	{
		...
		clearbufbyte(&_sync[i],sizeof(input),0L);
	...
	for(i=headspritestat[1];i>=0;i=nexti)
	{
		...
		if ((k&63) == 23)
		{
			...
			else
			{
				...
				doubvel = (TICSPERFRAME<<((_sync[target].bits&256)>0));
				xvect = 0, yvect = 0;
				if (_sync[target].fvel != 0)
				{
					xvect += ((((long)_sync[target].fvel)*doubvel*(long)sintable[(ang[target]+512)&2047])>>3);
					yvect += ((((long)_sync[target].fvel)*doubvel*(long)sintable[ang[target]&2047])>>3);
				}
				if (_sync[target].svel != 0)
				{
					xvect += ((((long)_sync[target].svel)*doubvel*(long)sintable[ang[target]&2047])>>3);
					yvect += ((((long)_sync[target].svel)*doubvel*(long)sintable[(ang[target]+1536)&2047])>>3);
				}
